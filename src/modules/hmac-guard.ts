...
import { NONCE_CHECK_LUA, decodeNonceResult } from '../lua-scripts.js';
import { compileGlob, LruMap, sha256Hex } from '../util.js';
...
export class HmacGuard {
  private readonly config: {
    ...
    secretResolver: NonNullable<HmacOptions['secretResolver']>;
    signedRoutes: ReadonlyArray<string>;
  };
  private readonly compiledSigned: ReadonlyArray<(subject: string) => boolean>;
  private readonly fallbackNonces: LruMap<string, number>;
  ...
  constructor(
    ...
    this.compiledSigned = this.config.signedRoutes.map((p) => compileGlob(p.toLowerCase()));
    ...
    this.fallbackNonces = new LruMap<string, number>(16_384);
  }

  /** Returns true iff the route's `${method} ${path}` matches a signed pattern,
   *  or if all routes are signed (empty `signedRoutes`). */
  private isSignedRoute(ctx: RequestContext): boolean {
    if (this.config.signedRoutes.length === 0) return true;
   ...
   */
  async verify(ctx: RequestContext): Promise<Result<HmacDecision, SecurityError>> {
    if (!this.isSignedRoute(ctx)) {
      return ok({ valid: true, rejectKind: 'unsigned_route', keyId: null });
    }
    ...
    if (driftMs > this.config.maxDriftMs) {
      return reject(
        'timestamp_drift',
        401,
        `timestamp drift ${driftMs}ms exceeds ${this.config.maxDriftMs}ms`,
      );
    }
    ...
    const claimRet = await this.redis.evalsha(NONCE_CHECK_LUA, [nonceKey], [
      String(this.config.nonceTtlMs),
      '1',
    ]);
    let nonceFresh = false;
    if (claimRet.ok) {
      nonceFresh = decodeNonceResult(claimRet.value);
    } else {
    ...
    if (!nonceFresh) {
      return reject(
        'replay_detected',
        403,
        `nonce reuse detected (key=${keyId})`,
      );
    }
    ...
    const serverDigest = createHmac('sha256', secret.secret).update(stringToSign, 'utf8').digest();
    ...
    return ok({ valid: true, rejectKind: null, keyId });
  }

  /** Maps a non-OK reject to a SecurityError (used by the orchestrator). */
  static rejectKindToError(
    kind: HmacRejectKind,
    message: string,
    ctx: RequestContext,
  ): SecurityError {
    let statusCode = 401;
    let errorKind: SecurityError['kind'] = 'signature_invalid';
    switch (kind) {
...
function reject(
  rejectKind: HmacRejectKind,
  statusCode: number,
  message: string,
): Err<SecurityError> {
  void rejectKind;
  void statusCode;
  return err({ kind: 'signature_invalid', message, statusCode, context: { rejectKind } });
}

function headerOf(
  h: Readonly<Record<string, string | string[] | undefined>>,
  name: string,
): string {
...
function compareHexB64(
  ...
  if (encoding === 'hex') {
    if (!/^[0-9a-fA-F]+$/.test(candidate)) {
      ...
      candidate.slice(0, serverDigest.length * 2).split('').forEach((c, i) => {
        if (i < candBuf.length) candBuf[i] = c.charCodeAt(0) & 0xff;
      });
    } else {
    ...
    if (padNeed > 0) norm = norm + '='.repeat(padNeed);
  ...
  if (candBuf.length !== serverBuf.length) {
    ...
    try {
      return timingSafeEqual(candBuf.subarray(0, minLen), serverBuf.subarray(0, minLen));
    } catch {
      return false;
    }
  }
  try {
    return timingSafeEqual(candBuf, serverBuf);
  } catch {
    return false;
  }
}
