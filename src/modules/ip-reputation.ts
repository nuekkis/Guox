...
import { canonicalizeIp, LruMap, nowSec } from '../util.js';
...
const DEFAULTS = {
  ...
  violationEvents: [
    ...
    'tls_spoofing',
    'schema_drift',
  ] as ReadonlyArray<SecurityEvent['kind']>,
};

interface BlockEntry {
  unblockAt: number; // epoch seconds
  source: 'memory' | 'redis';
}
...
export class IpReputation {
  private readonly config: {
    ...
    violationEvents: ReadonlyArray<SecurityEvent['kind']>;
  };
  private readonly blockMap: LruMap<string, BlockEntry>;
  /** Per-IP *consecutive* violation counter (in-memory first, mirrored to
   *  Redis asynchronously). */
  private readonly violationTimestamps = new LruMap<string, number[]>(4096);
  ...
  constructor(redis: RedisClient, opts?: IpReputationOptions) {
    ...
    this.blockMap = new LruMap<string, BlockEntry>(memCap);
  }

  public readonly _blockMapSizeForCompat = () => this.blockMap.size;

  /** Fast path: O(1) synchronous check. Never throws. */
  isBlocked(ctx: RequestContext): Result<IpReputationDecision, SecurityError> {
    const ip = canonicalizeIp(ctx.clientIp);
  ...
  async recordViolation(
    ctx: RequestContext,
    eventKind: SecurityEvent['kind'],
  ): Promise<IpReputationDecision> {
    ...
    if (luaRet.ok) {
      const r = decodeIpReputationResult(luaRet.value);
      if (r.blockedNow) {
  ...
  snapshot(): { blocked: number; violations: number } {
