 ...
 * ------------------------------------------------------------------ */
export const RATE_LIMIT_LUA = /* lua */ `
 ...
 * ------------------------------------------------------------------ */
export const NONCE_CHECK_LUA = /* lua */ `
local key = KEYS[1]
local ttlMs = ARGV[1]
local value = ARGV[2]
 ...
 * ------------------------------------------------------------------ */
export const IP_REPUTATION_LUA = /* lua */ `
 ...
 * ------------------------------------------------------------------ */
export const JWT_BLACKLIST_LUA = /* lua */ `
local key = KEYS[1]
...
export function decodeRateLimitResult(
  raw: unknown,
): RateLimitDecision {
  // ioredis returns table elements as strings by default.
  const arr = Array.isArray(raw) ? raw.map((n) => Number(n)) : [];
  const allowed = arr[0] === 1;
  const remaining = Number.isFinite(arr[1]) ? (arr[1] as number) : 0;
  const retryAfter = Number.isFinite(arr[2]) ? (arr[2] as number) : 0;
  const charged = Number.isFinite(arr[3]) ? (arr[3] as number) : 0;
  const limit = Number.isFinite(arr[4]) ? (arr[4] as number) : 0;
  return { allowed, limit, remaining, retryAfter, charged };
}

export function decodeNonceResult(raw: unknown): boolean {
...
export function decodeIpReputationResult(raw: unknown): {
  blockedNow: boolean;
  violations: number;
} {
  if (!Array.isArray(raw)) return { blockedNow: false, violations: 0 };
  return {
