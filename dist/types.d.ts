...
export type Result<T, E extends SecurityError = SecurityError> = Ok<T> | Err<E>;
export declare const ok: <T>(value: T) => Ok<T>;
export declare const err: <E extends SecurityError>(error: E) => Err<E>;
export type SecurityErrorKind = 'circuit_open' | 'rate_limited' | 'replay_detected' | 'signature_invalid' | 'timestamp_drift' | 'session_revoked' | 'session_hijack' | 'fingerprint_mismatch' | 'payload_oversize' | 'payload_malformed' | 'payload_too_deep' | 'prototype_pollution' | 'ip_blocked' | 'tls_spoofing' | 'schema_drift' | 'misconfigured';
...
export type IdentityStrategy = 'api_key' | 'jwt_sub' | 'ip'
...
export type SecretResolver = (keyId: string) => Promise<HmacClientSecret | null> | HmacClientSecret | null;
...
export interface FingerprintShieldOptions {
    ...
    readonly decryptFpClaim?: (encrypted: string) => Promise<FingerprintHash | null> | FingerprintHash | null;
    /** Encrypt the freshly-computed hash for embedding into the JWT. */
    readonly encryptFpClaim?: (hash: FingerprintHash) => Promise<string> | string;
...
export interface SchemaDriftOptions {
    ...
    readonly resolve?: (ctx: RequestContext) => ((body: unknown) => SchemaValidationResult) | null;
...
export type SecurityEventKind = 'circuit_breaker' | 'rate_limited' | 'replay_detected' | 'signature_invalid' | 'timestamp_drift' | 'session_hijack' | 'session_revoked' | 'fingerprint_mismatch' | 'prototype_pollution' | 'payload_too_deep' | 'ip_blocked' | 'tls_spoofing' | 'schema_drift';
...
export interface GuoxOptions {
    ...
    readonly disabled?: ReadonlyArray<'rateLimit' | 'hmac' | 'sessionShield' | 'sanitizer' | 'ja4' | 'ipReputation' | 'schemaDrift'>;
}
/**
...
export declare const Identity: (s: string) => Identity;
export declare const NonceHash: (s: string) => NonceHash;
export declare const Ja4Fingerprint: (s: string) => Ja4Fingerprint;
export declare const FingerprintHash: (s: string) => FingerprintHash;
