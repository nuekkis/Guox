...
class SessionShield {
    ...
    fingerprintOf(ctx) {
        return (0, types_js_1.FingerprintHash)((0, util_js_1.sha256Hex)(canonicalFingerprintString(ctx)));
    }
    /** Convenience wrapper around the integrator's `encryptFpClaim`. */
    async encryptFor(ctx) {
    ...
    async verify(ctx) {
        ...
        if (!claims) {
            ...
            return (0, types_js_1.ok)({ ok: true, reason: 'missing_claims' });
        }
        ...
        if (recordedFp === null) {
            ...
            return (0, types_js_1.ok)({ ok: true, reason: 'matched' });
        }
        const matches = (0, util_js_1.timingSafeStringEq)(liveFp, recordedFp);
        if (matches) {
            return (0, types_js_1.ok)({ ok: true, reason: 'matched' });
        }
        // Mismatch — proceed based on policy.
        if (this.config.onMismatch === 'flag') {
            return (0, types_js_1.ok)({ ok: true, reason: 'fingerprint_mismatch' });
        }
    ...
    async revoke(claims) {
        if (claims.jti && claims.exp) {
            ...
            try {
                ...
                await this.redis.evalsha(lua_scripts_js_1.JWT_BLACKLIST_LUA, [this.redis.key('revoked:jwt:' + claims.jti)], [
                    String(ttlSec),
                    '1',
                ]);
            }
        ...
        if (claims.sessionId) {
            ...
            try {
                await this.redis.evalsha(`return redis.call('DEL', KEYS[1])`, [this.redis.key('session:' + claims.sessionId)], []);
            }
            catch {
                /* ignored */
            }
    ...
    async associate(sessionId, fpHash, ttlSeconds) {
        try {
            await this.redis.evalsha(`redis.call('HSET', KEYS[1], 'fp', ARGV[1])
         redis.call('EXPIRE', KEYS[1], ARGV[2])
         return 1`, [this.redis.key('session:' + sessionId)], [fpHash, String(ttlSeconds)]);
        }
        catch {
            /* ignore best-effort */
        }
    ...
    /* -------- private -------- */
    async resolveRecordedFingerprint(claims, _ctx) {
        // Path A: JWT claim (stateless).
        const claimName = this.config.fpClaimName;
        const injectedClaim = claims[claimName];
        if (typeof injectedClaim === 'string' && injectedClaim.length > 0) {
            if (this.config.decryptFpClaim) {
                ...
                return (0, types_js_1.FingerprintHash)(injectedClaim);
            }
        }
        // Path B: session id -> Redis hash.
        if (claims.sessionId) {
            try {
                const retResult = await this.redis.evalsha(`local v = redis.call('HGET', KEYS[1], 'fp'); if v == false then return '' else return v end`, [this.redis.key('session:' + claims.sessionId)], []);
                if (retResult.ok) {
                    const ret = retResult.value;
                    if (typeof ret === 'string' && ret.length > 0) {
                        // ioredis resolves single-string HGET directly.
                        return (0, types_js_1.FingerprintHash)(ret);
                    }
                    if (Array.isArray(ret) && ret.length > 0 && ret[0] !== undefined) {
                        ...
                        if (s.length > 0)
                            return (0, types_js_1.FingerprintHash)(s);
                    }
                    if (ret !== null &&
                        typeof ret === 'object' &&
                        typeof ret.toString === 'function') {
                        const s = String(ret);
...
exports.SessionShield = SessionShield;
/* ------------------------------------------------------------------ *
 ...
 * ------------------------------------------------------------------ */
function canonicalFingerprintString(ctx) {
...
function normalizeAcceptLanguage(raw) {
    if (raw.length === 0)
        return '';
